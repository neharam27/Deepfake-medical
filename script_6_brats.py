"""
script_6_brats.py
Leakage-free deepfake detection pipeline for the BraTS (Brain Tumor Segmentation) dataset.

Dataset: BraTS (Multi-modal Brain Tumor MRI)
  URL: https://www.med.upenn.edu/cbica/brats/  or  https://www.kaggle.com/datasets/dschettler8845/brats-2021-task1
  Format: NIfTI (.nii or .nii.gz) volumes, multiple modalities (T1, T1ce, T2, FLAIR)
  Each patient folder contains one volume per modality.

Strategy: extract one representative 2-D axial slice per volume and use it as an image.
...
Expected layout:
  DATA_PATH/
    BraTS2021_00000/
      ...
      ...
  (or any nested layout – the script walks the whole tree for .nii / .nii.gz files)

Before running, set DATA_PATH to the root of your BraTS download.
"""
...
try:
    ...
    sys.exit("[ERROR] nibabel is required.  Run:  pip install nibabel")

# ============================================================
# USER CONFIGURATION
# ============================================================
DATA_PATH  = os.environ.get("BRATS_PATH", "./brats_data")
OUTPUT_DIR = "."
WORK_DIR   = "./work_brats"
...
# ============================================================
# STEP 0 – Convert NIfTI volumes → 2-D PNG slices & split
# ============================================================
print("=" * 70)
print("STEP 0: DATA LOADING & SPLITTING (BraTS)")
print("=" * 70)

def collect_nifti_paths(data_path):
    """Collect .nii and .nii.gz file paths recursively."""
    paths = []
...
def nifti_to_gray_slice(nii_path):
    """Load a NIfTI volume and return the middle axial slice as a uint8 grayscale array."""
    try:
        vol = nib.load(nii_path).get_fdata()
        # Select middle slice along the last (axial) axis
        if vol.ndim == 4:
            vol = vol[..., 0]          # take first time-point if 4-D
        mid = vol.shape[2] // 2
...
all_nii = collect_nifti_paths(DATA_PATH)
if len(all_nii) == 0:
    sys.exit(
        f"[ERROR] No .nii / .nii.gz files found under DATA_PATH='{DATA_PATH}'.\n"
        "Set DATA_PATH to the BraTS dataset root."
    )

print(f"Found {len(all_nii)} NIfTI volumes")

# Convert all volumes to PNGs in a raw folder, then split
RAW_DIR = os.path.join(WORK_DIR, "raw_slices")
os.makedirs(RAW_DIR, exist_ok=True)

raw_png_paths = []
print("Extracting 2-D slices from NIfTI volumes...")
for i, nii_p in enumerate(tqdm(all_nii, desc="NIfTI→PNG")):
    img = nifti_to_gray_slice(nii_p)
    if img is None:
        continue
    out = os.path.join(RAW_DIR, f"{i:06d}_{os.path.basename(nii_p).replace('.nii.gz','').replace('.nii','')}.png")
    cv2.imwrite(out, img)
    raw_png_paths.append(out)

print(f"Successfully extracted {len(raw_png_paths)} slices")

indices = np.arange(len(raw_png_paths))
...
def copy_and_resize(paths, dest_dir, img_size, desc):
    ...
    for i, p in enumerate(tqdm(paths, desc=desc)):
        ...
        cv2.imwrite(os.path.join(dest_dir, f"{i:06d}_{os.path.basename(p)}"), img)

copy_and_resize([raw_png_paths[i] for i in train_idx], TRAIN_DIR, IMG_SIZE_GAN, "Copy train")
copy_and_resize([raw_png_paths[i] for i in val_idx],   VAL_DIR,   IMG_SIZE_GAN, "Copy val")
copy_and_resize([raw_png_paths[i] for i in test_idx],  TEST_DIR,  IMG_SIZE_GAN, "Copy test")

assert not (set(os.listdir(TRAIN_DIR)) & set(os.listdir(VAL_DIR)))
assert not (set(os.listdir(TRAIN_DIR)) & set(os.listdir(TEST_DIR)))
assert not (set(os.listdir(VAL_DIR))   & set(os.listdir(TEST_DIR)))
...
class ImprovedGenerator(nn.Module):
    def __init__(self,latent_dim=100,img_size=64):
        super().__init__()
        self.fc=nn.Linear(latent_dim,512*4*4);self.bn_fc=nn.BatchNorm1d(512*4*4)
...
# STEP 1 – Train GAN
# ============================================================
print("=" * 70); print("STEP 1: GAN TRAINING"); print("=" * 70)
...
# ============================================================
print("=" * 70); print("STEP 2: GENERATE FAKE IMAGES"); print("=" * 70)
generator.eval()
...
# ============================================================
print("=" * 70); print("STEP 3: DETECTOR TRAINING"); print("=" * 70)
...
# STEP 4 – Evaluate
# ============================================================
print("=" * 70); print("STEP 4: FINAL EVALUATION"); print("=" * 70)
