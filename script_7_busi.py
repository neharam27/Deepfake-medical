"""
script_7_busi.py
Leakage-free deepfake detection pipeline for the BUSI dataset.

Dataset: Breast Ultrasound Images (BUSI)
  URL: https://scholar.cu.edu.eg/?q=afahmy/pages/dataset  or
       https://www.kaggle.com/datasets/aryashah2k/breast-ultrasound-images-dataset
  Images: PNG files organised by class folder (benign / malignant / normal)
  Each class folder also contains *_mask.png files – those are excluded.
...
Before running, set DATA_PATH to the root of your BUSI download.
"""
...
# USER CONFIGURATION
# ============================================================
DATA_PATH  = os.environ.get("BUSI_PATH", "./busi_data")
OUTPUT_DIR = "."
WORK_DIR   = "./work_busi"
...
# ============================================================
# STEP 0 – Collect BUSI images (exclude masks) & split
# ============================================================
print("=" * 70)
print("STEP 0: DATA LOADING & SPLITTING (BUSI)")
print("=" * 70)

def collect_busi_images(data_path):
    """Collect ultrasound images, skipping mask files (_mask.png)."""
    paths = []
    for root, _dirs, files in os.walk(data_path):
        for fname in files:
            lower = fname.lower()
            if lower.endswith(".png") and "_mask" not in lower:
                paths.append(os.path.join(root, fname))
    return sorted(paths)

all_paths = collect_busi_images(DATA_PATH)
if len(all_paths) == 0:
    sys.exit(
        f"[ERROR] No images found under DATA_PATH='{DATA_PATH}'.\n"
        "Set DATA_PATH to the BUSI dataset root."
    )

print(f"Found {len(all_paths)} images (masks excluded)")
