"""
script_8_covid19.py
Leakage-free deepfake detection pipeline for the COVID-19 Radiography Database.
...
Dataset: COVID-19 Radiography Database (Kaggle)
  ...
  Images: PNG chest X-rays organised in class folders:
    COVID/images/         (COVID-19 positive)
    Normal/images/        (Normal)
    Viral Pneumonia/images/
    Lung_Opacity/images/  (present in newer releases)

The script collects ALL images from ALL class folders (no label is needed –
we only train a real-vs-fake GAN detector, not a disease classifier).
...
Expected layout:
  DATA_PATH/
    ...
    Viral Pneumonia/
      images/       *.png
    Lung_Opacity/           (optional)
      images/       *.png

Before running, set DATA_PATH to the root of your COVID-19 Radiography download.
"""
...
# USER CONFIGURATION
# ============================================================
DATA_PATH  = os.environ.get("COVID19_PATH", "./covid19_radiography_data")
OUTPUT_DIR = "."
WORK_DIR   = "./work_covid19"
...
# ============================================================
# STEP 0 – Collect COVID-19 Radiography images & split
# ============================================================
print("=" * 70)
print("STEP 0: DATA LOADING & SPLITTING (COVID-19 Radiography)")
print("=" * 70)

def collect_covid19_images(data_path):
    """Walk data_path and collect PNG images (skip mask / metadata files)."""
    paths = []
    for root, _dirs, files in os.walk(data_path):
        for fname in files:
            lower = fname.lower()
            if lower.endswith(".png") and "mask" not in lower:
                paths.append(os.path.join(root, fname))
    return sorted(paths)

all_paths = collect_covid19_images(DATA_PATH)
if len(all_paths) == 0:
    sys.exit(
        f"[ERROR] No PNG images found under DATA_PATH='{DATA_PATH}'.\n"
        "Set DATA_PATH to the COVID-19 Radiography Database root."
    )
