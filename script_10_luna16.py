"""
script_10_luna16.py
Leakage-free deepfake detection pipeline for the LUNA16 dataset.

Dataset: LUNA16 (Lung Nodule Analysis 2016)
  URL: https://luna16.grand-challenge.org/  or  https://www.kaggle.com/datasets/fanbyprinciple/luna16
  Scans are stored as MetaImage (.mhd + .raw) files (one per patient).
  Annotations are in CSVFILES/ (candidates.csv, annotations.csv).

Strategy: for each .mhd volume extract the central axial slice as a 2-D greyscale PNG.
...
Before running, set DATA_PATH to the root of your LUNA16 download.
"""
...
try:
    ...
    sys.exit("[ERROR] SimpleITK is required.  Run:  pip install SimpleITK")

# ============================================================
# USER CONFIGURATION
# ============================================================
DATA_PATH  = os.environ.get("LUNA16_PATH", "./luna16_data")
OUTPUT_DIR = "."
WORK_DIR   = "./work_luna16"
...
# ============================================================
# STEP 0 – Convert LUNA16 MHD volumes → PNG slices & split
# ============================================================
print("=" * 70)
print("STEP 0: DATA LOADING & SPLITTING (LUNA16)")
print("=" * 70)

def collect_mhd_paths(data_path):
    """Recursively find all .mhd files."""
    paths = []
...
def mhd_to_gray_slice(mhd_path):
    """Load a MetaImage CT volume and return the central axial slice as uint8."""
    try:
        itk_img  = sitk.ReadImage(mhd_path)
        vol      = sitk.GetArrayFromImage(itk_img).astype(np.float32)  # (Z, Y, X)
        # Clamp Hounsfield units to a lung-friendly window [-1000, 400] then normalise
...
all_mhd = collect_mhd_paths(DATA_PATH)
if len(all_mhd) == 0:
    sys.exit(
        f"[ERROR] No .mhd files found under DATA_PATH='{DATA_PATH}'.\n"
        "Set DATA_PATH to the LUNA16 dataset root (containing subset0/, subset1/, …)."
    )

print(f"Found {len(all_mhd)} MHD volumes")

# Convert all volumes to PNGs first
RAW_DIR = os.path.join(WORK_DIR, "raw_slices")
os.makedirs(RAW_DIR, exist_ok=True)

raw_png_paths = []
print("Extracting central axial slices from CT volumes …")
for i, mhd_p in enumerate(tqdm(all_mhd, desc="MHD→PNG")):
    img = mhd_to_gray_slice(mhd_p)
    if img is None:
        continue
    base = os.path.splitext(os.path.basename(mhd_p))[0]
    out  = os.path.join(RAW_DIR, f"{i:06d}_{base}.png")
    cv2.imwrite(out, img)
