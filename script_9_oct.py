"""
script_9_oct.py
Leakage-free deepfake detection pipeline for the OCT (Optical Coherence Tomography) dataset.

Dataset: Retinal OCT Images (Labeled Optical Coherence Tomography (OCT) and Chest X-Ray Images)
  URL: https://www.kaggle.com/datasets/paultimothymooney/kermany2018  or
       https://data.mendeley.com/datasets/rscbjbr9sj/3
  Images: JPEG organised in train/test folders with disease-class sub-folders:
...
Expected layout:
  DATA_PATH/
    ...
    test/
      CNV/       *.jpeg
      ...
  (or any equivalent flat/nested layout – the script walks the full tree)

Before running, set DATA_PATH to the root of your OCT dataset download.
"""
...
# USER CONFIGURATION
# ============================================================
DATA_PATH  = os.environ.get("OCT_PATH", "./oct_data")
OUTPUT_DIR = "."
WORK_DIR   = "./work_oct"
...
# ============================================================
# STEP 0 – Collect OCT images & split 60/20/20
# ============================================================
print("=" * 70)
print("STEP 0: DATA LOADING & SPLITTING (OCT)")
print("=" * 70)

def collect_oct_images(data_path):
    """Walk data_path and collect all JPEG (and PNG) image files."""
    paths = []
...
all_paths = collect_oct_images(DATA_PATH)
if len(all_paths) == 0:
    sys.exit(
        f"[ERROR] No images found under DATA_PATH='{DATA_PATH}'.\n"
        "Set DATA_PATH to the OCT dataset root."
    )
...
def copy_and_resize(paths, dest_dir, img_size, desc):
    """OCT images may be RGB – convert to grayscale, resize, save as PNG."""
