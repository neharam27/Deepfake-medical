"""
script_2_chexpert.py
Leakage-free deepfake detection pipeline applied to CheXpert dataset.
  ...
  Expected layout after download:
    DATA_PATH/
      train/
        patient00001/
          study1/
            frontal.jpg  (or view1_frontal.jpg, etc.)
      valid/
        patient00001/
          study1/
            frontal.jpg
...
Only frontal-view JPEG/PNG images are collected.
Before running, set DATA_PATH to the root of your CheXpert download.
"""
...
# USER CONFIGURATION
# ============================================================
DATA_PATH  = os.environ.get("CHEXPERT_PATH", "./chexpert_data")
OUTPUT_DIR = "."
WORK_DIR   = "./work_chexpert"
...
# ============================================================
# STEP 0 – Collect & split CheXpert images
# ============================================================
print("=" * 70)
print("STEP 0: DATA LOADING & SPLITTING (CheXpert)")
print("=" * 70)

def collect_chexpert_images(data_path):
    """Collect frontal-view images from CheXpert train/ and valid/ folders."""
    paths = []
    for split_folder in ("train", "valid"):
        folder = os.path.join(data_path, split_folder)
        ...
        for root, _dirs, files in os.walk(folder):
            for fname in files:
                ...
                if lower.endswith((".jpg", ".jpeg", ".png")):
                    # prefer frontal views but accept all if none marked
                    if "frontal" in lower or lower.endswith((".jpg", ".jpeg", ".png")):
                        paths.append(os.path.join(root, fname))
    return sorted(paths)

all_paths = collect_chexpert_images(DATA_PATH)

if len(all_paths) == 0:
    sys.exit(
        f"[ERROR] No images found under DATA_PATH='{DATA_PATH}'.\n"
        "Set DATA_PATH to the root of your CheXpert download."
    )
...
def copy_and_resize(paths, dest_dir, img_size, desc):
    ...
    for i, p in enumerate(tqdm(paths, desc=desc)):
        ...
        cv2.imwrite(os.path.join(dest_dir, f"{i:06d}_{os.path.basename(p)}.png"), img)
...
for epoch in range(GAN_EPOCHS):
    ...
    for real_imgs in pbar:
        ...
        opt_d.zero_grad()
        loss_d = criterion(discriminator(real_imgs), torch.ones(bs,1,device=DEVICE)) + \
                 criterion(discriminator(generator(torch.randn(bs,LATENT_DIM,device=DEVICE)).detach()),
                           torch.zeros(bs,1,device=DEVICE))
        loss_d.backward(); opt_d.step()
        opt_g.zero_grad()
        loss_g = criterion(discriminator(generator(torch.randn(bs,LATENT_DIM,device=DEVICE))),
    ...
    def __init__(self, real_dir, fake_dir, img_size=256):
        ...
        for f in sorted(os.listdir(real_dir)):
            if f.lower().endswith(".png"):
                self.images.append(os.path.join(real_dir, f)); self.labels.append(0)
        for f in sorted(os.listdir(fake_dir)):
            if f.lower().endswith(".png"):
                self.images.append(os.path.join(fake_dir, f)); self.labels.append(1)
...
idx_all    = np.arange(len(val_ds))
lbl_arr    = np.array(val_ds.labels)
...
for epoch in range(DET_EPOCHS):
    ...
    with torch.no_grad():
        for imgs, lbls in iv_loader:
            imgs, lbls = imgs.to(DEVICE), lbls.to(DEVICE)
            logits = detector(imgs); probs = torch.softmax(logits,1)
            iv_preds.extend(torch.argmax(logits,1).cpu().numpy())
            iv_labels.extend(lbls.cpu().numpy()); iv_probs.extend(probs[:,1].cpu().numpy())
...
with open(csv_path, "w", newline="") as fh:
    w = csv.writer(fh)
    w.writerow(["","Predicted Real","Predicted Fake"])
    w.writerow(["Actual Real", tn, fp])
    w.writerow(["Actual Fake", fn, tp])
